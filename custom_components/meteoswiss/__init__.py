...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if data_source == DATA_SOURCE_OPENMETEO:
        ...
        if latitude is None or longitude is None and station_id:
            # Try to load coordinates from station metadata
            lat, lon = await _load_station_coordinates(station_id)
            if lat and lon:
                latitude = lat
                longitude = lon
                # Update entry data with coordinates
                new_data = dict(entry.data)
                new_data[CONF_LATITUDE] = latitude
                new_data[CONF_LONGITUDE] = longitude
                hass.config_entries.async_update_entry(entry, data=new_data)

        # Forecast coordinator (uses Open-Meteo with MeteoSwiss fallback)
        forecast_coordinator = MeteoSwissForecastCoordinator(
            hass,
            station_id=station_id,
            latitude=latitude,
        ...
        _LOGGER.info("Forecast coordinator created (Open-Meteo with MeteoSwiss fallback)")
